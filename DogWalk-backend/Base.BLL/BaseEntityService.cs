...
{
    public BaseEntityService(IUnitOfWork
        uoW, TRepository repository, IBLLMapper<TDalEntity, TBllEntity> Mapper) : base(uoW,
        repository, Mapper)
    {
    }
}
    ...
    where TDalEntity : class, IDomainEntityId<TKey>
{
    ...
    // public async Task<IEnumerable<TBllEntity>> GetOverThreeTables(string tableToInclude, TKey? userId, bool noTracking = true)
    // {
    //     return (await Repository.GetOverThreeTables(tableToInclude, userId)).Select(e => Mapper.Map(e));
    // }

    public async Task<int> LOLO(TKey id, TKey? userId = default)
    {
